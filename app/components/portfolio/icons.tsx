type IconProps = {
  size?: number;
  className?: string;
};

export function PortfolioBadgeIcon({ size = 16, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-hidden
    >
      <path
        d="M12 1.5H6C5.73478 1.5 5.48043 1.60536 5.29289 1.79289C5.10536 1.98043 5 2.23478 5 2.5V3.5H4C3.73478 3.5 3.48043 3.60536 3.29289 3.79289C3.10536 3.98043 3 4.23478 3 4.5V14C3 14.0919 3.02531 14.182 3.07315 14.2604C3.12099 14.3388 3.18952 14.4026 3.27121 14.4446C3.35289 14.4866 3.44459 14.5054 3.53621 14.4987C3.62784 14.492 3.71587 14.4603 3.79063 14.4069L7 12.1144L10.21 14.4069C10.2848 14.4601 10.3727 14.4917 10.4642 14.4983C10.5558 14.5048 10.6474 14.4861 10.7289 14.4441C10.8105 14.4021 10.879 14.3384 10.9268 14.2601C10.9746 14.1817 10.9999 14.0918 11 14V11.5431L12.2094 12.4069C12.2841 12.4603 12.3722 12.492 12.4638 12.4987C12.5554 12.5054 12.6471 12.4866 12.7288 12.4446C12.8105 12.4026 12.879 12.3388 12.9269 12.2604C12.9747 12.182 13 12.0919 13 12V2.5C13 2.23478 12.8946 1.98043 12.7071 1.79289C12.5196 1.60536 12.2652 1.5 12 1.5ZM12 11.0288L11 10.3125V4.5C11 4.23478 10.8946 3.98043 10.7071 3.79289C10.5196 3.60536 10.2652 3.5 10 3.5H6V2.5H12V11.0288Z"
        fill="currentColor"
      />
    </svg>
  );
}

export function ArrowTopRightIcon({ size = 40, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-hidden
    >
      <path
        d="M8.33333 29.3167L25.9833 11.6667H15V8.33333H31.6667V25H28.3333V14.0167L10.6833 31.6667L8.33333 29.3167Z"
        fill="currentColor"
      />
    </svg>
  );
}
